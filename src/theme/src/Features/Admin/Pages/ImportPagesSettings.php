...
/**
 * Clase para gestionar las páginas de importación en el panel de administración
 */
class ImportPagesSettings
{
	/**
	 * Constructor
	 */
	public function __construct()
	{
		...
		// Mantener el hook para procesamiento ACF
		add_action("acf/save_post", [$this, "processCsvProjectOnSave"], 20);
	}

	/**
	 * Registra la página de importación en el gestor de páginas.
	 *
	...
	/**
	 * Procesa el archivo CSV al guardar
	 *
	 * @param string $post_id ID del post
	 * @return void
	 */
	public function processCsvProjectOnSave($post_id): void
	{
		if ($post_id !== "options") {
		...
		if ($project_file_data = get_field("options_page_import_csv_project_post", "option")) {
			...
			if ($project_file_path && file_exists($project_file_path) && !$already_processed) {
				...
				error_log(
					"process_csv_project_on_save: Import message prepared: " . $project_message
				);
